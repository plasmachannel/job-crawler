...
rm job-profile-uploader.zip
npm install --omit=dev
npm prune --omit=dev

# Step 1: Zip the files in the directory
echo "Zipping files..."
zip -r job-profile-uploader.zip jobProfileUploaderHandler.js jobProfileUploader/ common/ node_modules/ package.json
if [ $? -ne 0 ]; then
...
S3_BUCKET="s3://job-search-lambda-functions/job-profile-uploader.zip"
echo "Uploading to S3 bucket: $S3_BUCKET"
aws s3 cp job-profile-uploader.zip $S3_BUCKET
if [ $? -ne 0 ]; then
...
aws lambda update-function-code --function-name $LAMBDA_ARN --s3-bucket "job-search-lambda-functions" --s3-key "job-profile-uploader.zip"
if [ $? -ne 0 ]; then
